...
parser.add_argument("--method", type=str, required=True,
                    choices=["uc_safe", "future_only", "parse_chain"],
                    help='method used to rebuild the slashing protection '
                    'information. The "parse_chain" option is experimental, '
                    'and must be used with argument "--validator-index '
                    'VALIDATOR_INDEX" (a single validator index input).')
...
val_input_group.add_argument("--validator-index", type=int, nargs='+',
                             help="index(es) of validator(s) for which to "
...
if METHOD == "parse_chain":
    if not VAL_INDEX:
        parser.error('The "--method parse_chain" argument can only be used '
                     'with the "--validator_index" option')
    elif len(VAL_INDEX) > 1:
        parser.error('The "--method parse_chain" option can only be used with '
                     'a single index input for the "--validator_index" option')
